...
thread_local bool trunning = true;
thread_local uint32_t valid = 0;
...
create_connector(pen_event_t ev, pen_connector_t *self, pen_speed_t *speeder)
{
    ...
    assert(pen_event_add_rw(ev, (pen_event_base_t*)self));
    valid ++;
}
...
thread_local uint32_t num = 0;
thread_local uint16_t gp = 0;
...
_on_read(pen_event_base_t *eb)
{
    ...
    self->offset_ = 0;
    assert(strncmp(PONG, self->buf_, PONG_SIZE) == 0);
...
_on_write(pen_event_base_t *eb)
{
    ...
    assert(write(eb->fd_, "ping", 4) == 4);
    assert(pen_event_mod_r(self->ev_, eb));
    return true;
}
...
main(int argc, char *argv[])
{
    ...
    assert(pen_thread_pool_start(tpool));
    start_server(ev);
